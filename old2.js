...
const server = new WebSocketServer({ port: 2222 });
    ...
    switch (data.id) {
      case 2:
        ...
        if (list_queue.length >= 2) {
          ...
          list_queue[0].send(JSON.stringify({
            "id": 2, "playerTh": 1,
          ...
          list_queue[1].send(JSON.stringify({
            "id": 2, "playerTh": 2,
      ...
      case 20: /// player join custom, room
        ...
        if (listCustomroom[targetID].players.length <= 1) {
